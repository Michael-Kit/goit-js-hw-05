...
// Напиши функцію calcAverageCalories(days), яка повертає середньодобове значення кількості калорій, які спортсмен споживав
// протягом тижня. Функція очікує один параметр: days — масив об’єктів. Кожен об’єкт описує день тижня та кількість калорій
// calories, спожитих спортсменом, у цей день. Візьми код нижче і встав після оголошення своєї функції для перевірки коректності
// її роботи. У консоль будуть виведені результати її викликів.

function calcAverageCalories(days) {
  // Перевірка наявності днів
  if (days.length === 0) {
  ...
  // Обчислення середньодобового значення
  return Math.round(totalCalories / days.length);
}       
// Перевірка функції з різними наборами даних
console.log(
  calcAverageCalories([
